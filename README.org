...
This project aims to remove as much code as possible from Emacs—that means breaking all compatibility features for different operating systems and only support macOS. Also to remove all GUI related code so it only works on the terminal.

Because compatibility is not an issue, I plan on also removing all built in packages that can be installed separately.

This is NOT meant for public usage—it's a personal experiment and building a tool for my own use.
